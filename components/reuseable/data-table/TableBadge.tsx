...
export function TableBadge({ children, variant }: BadgeProps) {
  ...
  const map: Record<string, string> = {
    ...
    MainEvent: "border-[#F6D642] bg-[#F6D6421A] text-[#F6D642]",
  ...
  return (
    <span
      className={`px-3 py-1 rounded-[32px] border border-solid text-sm font-medium leading-[150%] font-inter inline-block whitespace-nowrap capitalize ${
        map[key] || "border-white/30 bg-white/10 text-white"
      }`}
    >
      {children}
    </span>
